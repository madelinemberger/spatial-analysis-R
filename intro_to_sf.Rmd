...
system.time(ak_shp_rgdal <- readOGR(dsn="shapefiles", layer="ak_regions")) #dsn is the folder you wants, layer will pull anything with that beginning
object.size(ak_shp_rgdal) #this tells you how bit something is, ie if its taking too long to load
plot(ak_shp_rgdal)
```
...
#this stores spatial data in a data frame format

system.time(ak_shp_sf <- read_sf("shapefiles/ak_regions.shp")) #tells you how long it takes to read something in 
...
```{r}
st_crs(ak_shp_sf) #this will tell you what CRS this is in

#EPSG - code assigned to standard projection, uses latitude and longitude. how you reference your point to the Earth
#can see that its using WGS84
#projection is how you squash the datum to a horizontal plane
#if you have two shapefiles, you want CRS to be the same 
```
...
```{r}
#transform or reproject your spatial data inti a new CRS

ak_shp_sf <- ak_shp_sf %>% #this is a way to overwrite what you read it 
  st_transform(crs = 3338) # you just need the EPSG code here

st_crs(ak_shp_sf) # can see that units are now in meters, which is easer to work with than degrees

#these projects are equal area, instead of equal distance. you can never use both 
```

```{r}
plot(ak_shp_sf) #plotting each of the columns differently (there were three columns), this is why it takes awhile. below we will learn how to plot just one feature from one column
...
```{r}
nrow(ak_shp_sf) #how many rows
ncol(ak_shp_sf) #how many columns 

#sticky geometries is what allows this, see Allison's art!
#Sticky geometries
...
#can see "stickiness" here, you only selected one column but the geometry column will stay with you, since it is fundamental to your data frame
#if your doing geospatial analysis you always want geometry anyway 
```
...
class(pop) #this is a dataframe, needs to be an sf object as well to join
```
...
```{r}
pop_sf <- st_as_sf(pop, 
                  coords = c('lng', 'lat'), #need to give it coordinates, name of cols,x y
                  crs = 4326, #need to assign a coordinate refernce system
                  remove = F) #this overrides default, which would turn lat lon into geom, and get rid of those columns, we don't want to do this right now

head(pop_sf)

plot(pop_sf["population"]) #this will just plot ONE column 
```
...
```{r, eval = F}
pop_joined_sf <- st_join(pop_sf, ak_shp_sf, join = st_within) #join function tells you how to join, there are st_intersect, etc. we have points within polygons, so we want to use st_within 
```
...
```{r}
pop_sf <- st_transform(pop_sf, crs = 3338) #needs to match
#need to accept data as it is, and then transform it (don't quite understand), have to say where they are on the earth, and then you can transform you can do it 
...
#we put pop on left because we wanted a region for every city, default of join 
#if you wanted all the cities in a region, would use a contain (so this really depends on what question you are asking)

head(pop_joined_sf)
plot(pop_joined_sf["region"]) #can see that now, all your points have a region assigned to them 
```
...
pop_region <- pop_joined_sf %>% 
  as.data.frame() %>% #this will drop sticky geometry 
  group_by(region) %>% 
  summarise(total_pop = sum(population)) #note spelling 
...
```{r}
ak_pop_sf <- left_join(ak_shp_sf, pop_region) #there was a more elegant way of doing this 
...
#what this did is that it unioned all the grouped geometries into one so now this object, instead of being 13 rows, is now 4. Can see this when you look at df, "multipolygon"
...
```{r plot}
write_sf(ak_mgmt, "shapefiles/ak_mgmt.shp", delete_layer = TRUE)
```
...
```{r}
#simplest plot
ggplot(ak_pop_sf) +  #this lets you plot a spatial object in ggplot
  geom_sf()   #this is the new geom 
```
...
```{r}
ggplot(ak_pop_sf) +
  geom_sf(aes(fill = total_pop)) #tell what column R should use to fill the polygons
```
...
ggplot(ak_pop_sf) +
  ...
  scale_fill_continuous(low = "khaki", high =  "firebrick", labels = comma) #Use this for other graphs!! super helpful, this is from scales package 
```
...
```{r}
ggplot() + #so you can plot all these shapefile on top of each other 
  geom_sf(data = ak_pop_sf, aes(fill = total_pop)) +
  geom_sf(data = rivers, aes(size = StrOrder), color = "black") + #aes tells you what data you want to control size of the data point 
  geom_sf(data = pop_sf, aes(), size = .5) + #0.5 tells you size of dots
  scale_size(range = c(0.01, 0.2), guide = F) + #scale the default size of rivers data, play around until you get one that you like. guide = F turns off legend
  theme_bw() +
  labs(fill = "Total Population") +
